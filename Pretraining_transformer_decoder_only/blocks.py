...
# compartan una misma “fuente de verdad”.
# =============================================================================
@dataclass
class MTConfig:
    vocab_size: int
...
def skew(QEr: torch.Tensor) -> torch.Tensor:
    ...
    # 2) Reinterpretamos memoria para desplazar los elementos por filas/columnas.
...
class RelativeMaskedMHA(nn.Module):
    ...
    def __init__(self, cfg: MTConfig):
        ...
        # Proyección conjunta para Q, K, V (estilo Transformer estándar).
        ...
        # Embeddings relativos para distancias en [-L+1, L-1], con L = max_seq_len.
    ...
    def forward(self, x: torch.Tensor, return_attn: bool = False) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        #    - Extraemos Er (2T-1, Dh) centrado respecto a max_seq_len.
