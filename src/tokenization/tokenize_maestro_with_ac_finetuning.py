...
Notas importantes:
- Usa tokenizer.encode(..., attribute_controls_indexes=...) en vez de tokenize_dataset(),
  porque aquí queremos controlar explícitamente los AC.
...
- El conteo de barras para los AC bar-level se obtiene de una tokenización preliminar SIN AC,
  contando los tokens Bar_None. Así evitamos depender de utilidades internas.
"""
...
OVERWRITE = True
SAVE_TOKEN_STRINGS_PREVIEW = False  # True si quieres guardar unas pocas tokens legibles por JSON
...
def is_bar_level_ac(ac_obj: Any) -> bool:
    """
    MidiTok documenta AC bar-level y track-level, pero aquí evitamos depender de internals frágiles.
    Heurística robusta:
...
def count_bars_per_track_from_pretokenization(tokenizer: REMI, score_preprocessed: Score) -> list[int]:
    ...
    tiene cada pista. Eso nos da la cantidad de barras sobre las que insertar AC bar-level.
    """
    ...
    for seq in seqs:
        ...
        # Si por alguna razón no aparece ningún Bar_None, dejamos al menos 1
...
def process_one_file(midi_path_str: str) -> FileResult:
    ...
    try:
        ...
        if len(seqs_list) == 1:
            ...
            # Guardamos manualmente una estructura simple si hay varias pistas.
            payload = {
