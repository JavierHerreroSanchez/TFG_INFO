...
# ATTRIBUTE CONTROLS
# ============================================================
def _get_num_bars(score: Score) -> int:
    """
    Estima el número de compases a partir de los marcadores de barra creados
    por el preprocesado/tokenizer.
    """
    # MidiTok suele exponer el número de compases a través de ticks de barra
    # con métodos privados. Probamos varias opciones robustas.
    for attr_name in ("_score_ticks_to_bars", "_get_bars_ticks"):
        fn = getattr(TOKENIZER, attr_name, None)
        if fn is None:
            continue
        try:
            bars = fn(score)
    ...
    # Fallback conservador: si falla, al menos un compás
    return 1


def build_all_ac_indexes(score: Score) -> dict[int, dict[int, object]]:
    """
    Pide a MidiTok que calcule TODOS los attribute controls
    para TODOS los tracks.

    Para los bar-level AC, bars_idx=None => todos los compases.
    Para los track-level AC, None también evita complicaciones y
    deja que MidiTok compute el control para el track completo.
    """
    attribute_controls = getattr(TOKENIZER, "attribute_controls", None)
    if not attribute_controls:
        return {}

    ac_indexes: dict[int, dict[int, object]] = {}
    for track_idx, _track in enumerate(score.tracks):
        ac_indexes[track_idx] = {
            ac_idx: None for ac_idx, _ac in enumerate(attribute_controls)
        }
...
def process_one(midi_str: str) -> tuple[str, str, str]:
    ...
    try:
        ...
        # IMPORTANTE:
        # preprocesamos antes para que los índices de barras/tracks de los AC
        # correspondan exactamente al score que luego se tokeniza.
        TOKENIZER.preprocess_score(score)
        ac_indexes = build_all_ac_indexes(score)
...
def main():
    ...
    # Mantengo tu lógica. Si quieres solo MAESTRO, deja activo solo maestro en DATASETS_TO_SCAN.
