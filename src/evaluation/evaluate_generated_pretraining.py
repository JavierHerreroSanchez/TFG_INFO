...
# Pool local de referencias para la puntuación por pieza.
...
def build_feature_table(files: List[Path], tag: str) -> pd.DataFrame:
    ...
    if use_parallel:
        with ProcessPoolExecutor(max_workers=NUM_WORKERS) as ex:
            futures = {ex.submit(extract_features, path): path for path in files}
...
def select_reference_pool(duration_beats: float, ref_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ref_sub = ref_df.loc[np.isfinite(ref_df["duration_beats_custom"].to_numpy(dtype=float))].copy()
    if ref_sub.empty:
        return ref_df
    ...
    pool = ref_sub[
        (ref_sub["duration_beats_custom"] >= low)
        & (ref_sub["duration_beats_custom"] <= high)
    ].copy()
...
def parse_pitch_hist_matrix(hist_jsons: pd.Series) -> np.ndarray:
    """Convierte histogramas JSON de pitch-class en una matriz (N, 12)."""

    rows = []
    for value in hist_jsons.astype(str):
        try:
            hist = np.asarray(json.loads(value), dtype=float)
            if hist.shape != (12,) or not np.isfinite(hist).all() or hist.sum() <= 0:
                hist = np.zeros(12, dtype=float)
        except Exception:
            hist = np.zeros(12, dtype=float)
        rows.append(hist)
    return np.vstack(rows) if rows else np.empty((0, 12), dtype=float)
...
def per_piece_reference_based(gen_row: pd.Series, ref_df: pd.DataFrame, ref_pitch_hists: Optional[np.ndarray] = None) -> Dict:
    ...
    for feat in used_features:
        if feat not in ref_df.columns:
            continue
        ref_vals = finite_values(ref_df[feat].to_numpy(dtype=float))
        if ref_vals.size == 0:
            continue
    ...
    pitch_hist_similarity_score = np.nan
    if ref_pitch_hists is not None and ref_pitch_hists.size:
        try:
            gen_hist = np.asarray(json.loads(str(gen_row["_pitch_hist_json"])), dtype=float)
            if gen_hist.shape == (12,) and np.isfinite(gen_hist).all() and gen_hist.sum() > 0:
                gen_hist = gen_hist / gen_hist.sum()
                pitch_hist_similarity_score = float(np.minimum(ref_pitch_hists, gen_hist[None, :]).sum(axis=1).mean())
        except Exception:
            pitch_hist_similarity_score = np.nan
    elif "_pitch_hist_json" in ref_df.columns:
        pitch_hist_sims = []
    ...
    if np.isfinite(base_score) and np.isfinite(pitch_hist_similarity_score):
        # Más duro: 90% features, 10% histograma
    ...
    return {
        ...
        "matched_reference_count": int(len(ref_df)),
...
def range_acceptance_score(x: float, lo: float, hi: float, softness: float | None = None) -> float:
    ...
    Fuera del rango, caída más dura.
    """
    if not np.isfinite(x):
        return np.nan
...
def main():
    ...
    for _, gen_row in gen_df.iterrows():
        reference_pool = select_reference_pool(float(gen_row["duration_beats_custom"]), ref_df)
        ref_pitch_hists = parse_pitch_hist_matrix(reference_pool["_pitch_hist_json"]) if "_pitch_hist_json" in reference_pool.columns else None
        ref_based = per_piece_reference_based(gen_row, reference_pool, ref_pitch_hists)
    ...
    # Unificar la salida CSV en menos archivos
    summary_rows = []
    summary_rows.append({
        "section": "meta",
        "key": "generated_dir",
    ...
    summary_rows.append({
        "section": "meta",
        "key": "n_generated_files",
        "value": int(len(gen_df)),
    })
    summary_rows.append({
        "section": "meta",
        "key": "n_reference_files",
        "value": int(len(ref_df)),
    })
    summary_rows.append({
        "section": "meta",
        "key": "reference_scoring_scope",
        "value": REFERENCE_SCORING_SCOPE,
    })
    summary_rows.append({
        "section": "meta",
        "key": "local_ref_duration_tol",
        "value": float(LOCAL_REF_DURATION_TOL),
    })
