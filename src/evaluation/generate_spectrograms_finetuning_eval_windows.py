...
"""
Evaluación espectral para MIDIs generados por FINETUNING usando ventanas de referencia.

Versión conservadora del evaluador original:
- Mantiene la misma lógica general:
  * espectrogramas PNG solo para los generados
  * referencias usadas para comparación y scoring
  * score por pieza + OA/KLD globales
  * mismos pesos reference-based / reference-free
  * mismas métricas espectrales
- Diferencia principal:
  * las referencias de finetuning no se evalúan como piezas completas para el score,
    sino como ventanas de audio con duración comparable a las piezas generadas.
  * esto evita comparar fragmentos generados cortos contra sonatas completas.

Salidas:
- spectral_features_all.csv:
    generado + ventanas de referencia en un único CSV.
- spectral_reference_full_piece_features.csv:
    referencias completas, solo diagnóstico.
- spectral_reference_window_features.csv:
    ventanas de referencia usadas para scoring.
- spectral_evaluation.csv:
    score por pieza + bloque global compacto.
...
# ============================================================
GENERATED_DIR = Path(r"C:\Users\herre\PycharmProjects\TFG_INFO\output\generation_finetuning_tfg_third\batch_1")
OUT_DIR = Path(r"/output/generation_finetuning_tfg_third/batch_1/best_train/midi_spectral_eval_windows")
...
# Solo para la calidad visual de los PNG
DPI = 180
FIG_W = 14
FIG_H = 5.5
COLORMAP = "magma"
...
def main():
    ...
    print(f"[DEBUG] GENERATED_DIR exists: {GENERATED_DIR.exists()}")
    print(f"[DEBUG] REFERENCE_DIR exists: {REFERENCE_DIR.exists()}")
    print(f"[DEBUG] OUT_DIR parent exists: {OUT_DIR.parent.exists()}")
    ...
    print(f"[DEBUG] generated files found: {len(generated_files)}")
    print(f"[DEBUG] first generated files: {generated_files[:5]}")
