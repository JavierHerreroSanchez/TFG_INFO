...
# Métricas más robustas a longitud: pesan más en la puntuación.
...
# Métricas reference-free: heurísticas internas, sin usar referencia.
...
def main() -> None:
    ...
    for _, gen_row in gen_df.iterrows():
        ...
        per_piece_rows.append({
            ...
            "issues": " | ".join(issues),
            "reference_based_details_json": json.dumps(ref_based["details"], ensure_ascii=False),
            "reference_free_details_json": json.dumps(ref_free["details"], ensure_ascii=False),
        })

    per_piece_df = pd.DataFrame(per_piece_rows).sort_values("global_score", ascending=False)
    ...
    gen_df.to_csv(OUT_DIR / "generated_features.csv", index=False, encoding="utf-8-sig")
    ref_df.to_csv(OUT_DIR / "reference_features.csv", index=False, encoding="utf-8-sig")
    per_piece_df.to_csv(OUT_DIR / "per_piece_evaluation.csv", index=False, encoding="utf-8-sig")
    global_report.to_csv(OUT_DIR / "global_reference_based_report.csv", index=False, encoding="utf-8-sig")
