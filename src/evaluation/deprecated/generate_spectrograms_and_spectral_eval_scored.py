...
"""
Evaluación espectral para MIDIs generados, basada en audio sintetizado SIN SoundFont.
...
4) Si USE_REFERENCE=True:
   ...
   - calcula score spectral reference-free
   - calcula score spectral global por pieza
   - calcula OA/KLD globales entre generado y referencia
5) Guarda CSVs compactos y un JSON de detalles
"""
...
MAX_REFERENCE_FILES = 300
MAESTRO_FRACTION = 0.50
REFERENCE_RANDOM_SEED = 42

# Pool local por duración
...
def per_piece_reference_free(gen_row: pd.Series) -> Dict:
    """
    Reglas sencillas y duras sobre descriptores espectrales.
    Son heurísticas para plausibilidad espectral en piano sintetizado simple.
    """
    checks = {
        "rms_mean": range_acceptance_score(safe_float(gen_row.get("rms_mean", np.nan)), 0.02, 0.22),
        "spectral_centroid_mean": range_acceptance_score(safe_float(gen_row.get("spectral_centroid_mean", np.nan)), 300.0, 2200.0),
        "spectral_bandwidth_mean": range_acceptance_score(safe_float(gen_row.get("spectral_bandwidth_mean", np.nan)), 400.0, 2600.0),
        "spectral_rolloff_mean": range_acceptance_score(safe_float(gen_row.get("spectral_rolloff_mean", np.nan)), 800.0, 5000.0),
        "spectral_flatness_mean": range_acceptance_score(safe_float(gen_row.get("spectral_flatness_mean", np.nan)), 0.0005, 0.08),
        "zcr_mean": range_acceptance_score(safe_float(gen_row.get("zcr_mean", np.nan)), 0.005, 0.15),
        "spectral_contrast_mean": range_acceptance_score(safe_float(gen_row.get("spectral_contrast_mean", np.nan)), 8.0, 28.0),
        "mel_energy_mean": range_acceptance_score(safe_float(gen_row.get("mel_energy_mean", np.nan)), 0.0001, 8.0),
    }
...
# ============================================================
def main():
    print("[INFO] Iniciando evaluación espectral de MIDIs generados")
    ...
    if len(generated_files) == 0:
        raise RuntimeError("No se encontró ningún MIDI generado. Revisa GENERATED_DIR.")

    gen_spec_dir = OUT_DIR / "generated_spectrograms"
    ref_spec_dir = OUT_DIR / "reference_spectrograms"

    gen_df = build_feature_table(generated_files, gen_spec_dir, "generated")
    if gen_df.empty:
    ...
    if USE_REFERENCE:
        ...
        ref_df = build_feature_table(reference_files, ref_spec_dir, "reference")
        if ref_df.empty:
    ...
    generated_csv = OUT_DIR / "generated_spectral_features.csv"
    reference_csv = OUT_DIR / "reference_spectral_features.csv"
    per_piece_csv = OUT_DIR / "spectral_per_piece_evaluation.csv"
    global_csv = OUT_DIR / "spectral_global_report.csv"
    details_json = OUT_DIR / "spectral_per_piece_details.json"
    summary_json = OUT_DIR / "spectral_summary.json"

    for p in [generated_csv, reference_csv, per_piece_csv, global_csv, details_json, summary_json]:
        if p.exists():
            p.unlink()

    gen_df.to_csv(generated_csv, index=False, encoding="utf-8-sig")
    per_piece_df.to_csv(per_piece_csv, index=False, encoding="utf-8-sig")

    if USE_REFERENCE and not ref_df.empty:
        ref_df.to_csv(reference_csv, index=False, encoding="utf-8-sig")
