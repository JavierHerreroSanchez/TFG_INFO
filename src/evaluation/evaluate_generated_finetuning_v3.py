...
"""
Evaluación symbolic MIDI para FINETUNING usando ventanas de referencia.

Versión conservadora: mantiene el script de finetuning original casi intacto.
La diferencia principal es que las referencias completas se cortan en ventanas
comparables a la duración de las piezas generadas antes de calcular el score.

Objetivo:
- No inflar artificialmente la score cambiando muchas métricas.
- Corregir el error principal: comparar fragmentos generados contra sonatas completas.
- Mantener GLOBAL_FEATURES, used_features, pesos y CSV principal lo más parecidos posible
  al evaluador original de finetuning.

Salidas:
- reference_features.csv: features de ventanas de referencia, usadas para scoring.
- reference_full_piece_features.csv: features de piezas completas, solo diagnóstico.
- per_piece_evaluation.csv: mismo esquema principal que el script original.
- summary_compact.csv
- per_piece_details.json: incluye detalles extra de la ventana local usada.
"""
...
# Se mantienen los mismos pesos que en el script original para que la comparación
# con pretraining sea lo más justa posible. La mejora debe venir de comparar contra
# ventanas, no de cambiar la ponderación.
...
# ============================================================
# CONFIGURACION DE VENTANAS DE REFERENCIA
# ============================================================
...
# Ajustes mínimos para métricas problemáticas, sin cambiar el conjunto de métricas.
# 1) scale_consistency: en el reference-based original se penaliza también si es más alta
#    que la media. Eso puede ser injusto: mayor consistencia tonal no debería ser peor.
# 2) sigma cero: si todas las ventanas tienen el mismo valor, evitamos que cualquier
#    diferencia caiga automáticamente al mínimo por dividir entre eps.
...
def strict_feature_score(x: float, ref_vals: np.ndarray, eps: float = 1e-8) -> float:
    ...
    # Mínimo ajuste frente al original: si sigma es cero o casi cero, no usamos eps
    # porque cualquier diferencia pasa a z enorme. Usamos una escala natural de 1 unidad.
    # Esto afecta sobre todo a contadores discretos como n_pitch_classes_used.
    if sigma < eps:
...
def feature_score(feat: str, x: float, ref_vals: np.ndarray) -> float:
    if USE_RANGE_FOR_SCALE_CONSISTENCY and feat == "scale_consistency":
        ...
        # de un rango musicalmente razonable. Esto corrige el caso visto en tu JSON:
...
def per_piece_reference_based(gen_row: pd.Series, local_ref_df: pd.DataFrame) -> Dict:
    # Se mantiene exactamente el conjunto de métricas del script original.
    used_features = [
        "pitch_range",
        "n_pitch_classes_used",
        "n_pitches_used",
...
def per_piece_reference_free(gen_row: pd.Series) -> Dict:
    # Se mantiene igual que el script original.
    checks = {
...
def main():
    ...
    # Reporte diagnóstico: generado vs sonatas completas, para comprobar el sesgo original.
