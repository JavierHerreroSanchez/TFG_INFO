...
def cache_config() -> CacheConfig:
    """Agrupa parametros de datos/cache para reutilizar el flujo comun."""
    return CacheConfig(
        index_csv=INDEX_CSV,
...
def model_config() -> MTModelConfig:
    """Mantiene la arquitectura compatible con el checkpoint de pretraining."""
...
def load_pretrained_checkpoint(model: torch.nn.Module, ckpt_path: Path, device: str) -> None:
    """Carga el checkpoint base de pretraining antes de ajustar al corpus objetivo."""
...
def export_listen_samples(
    ...
    """Genera muestras JSON desde primers fijos para escuchar progreso entre evaluaciones."""
...
def train_one_update(model, opt, scaler, autocast_dtype, train_iter, train_loader):
    """Ejecuta un update de finetuning con gradient accumulation."""
    accum_loss = 0.0
...
def maybe_log_progress(update: int, total_updates: int, loss: float, lr: float, tokens_per_update: int, start_time: float) -> None:
    """Imprime progreso de entrenamiento a intervalos fijos."""
...
def evaluate_and_checkpoint(update: int, total_updates: int, model, val_loader, opt, scaler, cfg: MTModelConfig, cache, state: dict) -> bool:
    """Evalua validacion, guarda checkpoints/muestras y devuelve True si hay early stop."""
...
def evaluate_test(model, test_loader) -> None:
    """Evalua el conjunto de test usando el mejor checkpoint si esta disponible."""
...
def main():
    ...
    model.train()
    early_state = {"best_val": float("inf"), "best_update": -1, "patience_count": 0}
    update = 0
    train_iter = iter(train_loader)
    t0 = time.time()
    ...
    while update < total_updates:
        ...
        # El LR se actualiza por update, no por micro-batch.
        accum_loss, train_iter = train_one_update(model, opt, scaler, autocast_dtype, train_iter, train_loader)
        update += 1

        maybe_log_progress(update, total_updates, accum_loss, lr, tokens_per_update, t0)
        if evaluate_and_checkpoint(update, total_updates, model, val_loader, opt, scaler, cfg, cache, early_state):
            break

    print("[DONE] finetuning terminado.")
    print(f"[DONE] best_val={early_state['best_val']:.4f} @ update={early_state['best_update']}")
